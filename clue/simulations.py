...
def apply_matrix(simulation: OdeResult, matrix: ndarray | SparseRowMatrix):
    ...
    if len(matrix.shape) != 2:
        raise TypeError(
            "The 'matrix' argument is not valid: it does not have a matrix shape"
        )
    elif matrix.shape[1] != len(simulation.y):
        raise TypeError(
            f"The 'matrix' argument is not valid: wrong number of columns [got {matrix.shape[1]}, expected {len(simulation.y)}]"
        )
            ...
            if all(el == 0 for el in row):
                ...
                for i in range(len(row)):
                    ...
                    if c != 0 and len(parts) == 0:
                        parts.append(
                            f"{'-' if c < 0 else ''}{'' if c in (1, -1) else f'{abs(c)}*'}{v}"
                        )
                    elif c != 0:
                        parts.append(
                            f"{'+ ' if c > 0 else '- '}{'' if c in (1,-1) else f'{abs(c)}*'}{v}"
...
def _merge_simulations(sim1: OdeResult, sim2: OdeResult):
    if (sim1.t == sim2.t).all():
        ...
        if all(hasattr(sim, "names") for sim in (sim1, sim2)):
            kwds["names"] = list(sim1.names) + list(sim2.names)
        return OdeResult(**kwds)
...
def compare_simulations(sim1: OdeResult, sim2: OdeResult, measures=[]):
    ...
    * ``"l1"``: a normalized L1 norm will be computed to compare the two simulations.
    ...
    names = (
        sim1.names
        if hasattr(sim1, "names")
        else sim2.names if hasattr(sim2, "names") else None
    )

    # Creating object for comparison
    sim = OdeResult(
        t=new_t, y=new_y, status=new_status, message=new_message, success=success
    )
...
def create_figure(
    ...
    * ``kwds``: optional arguments that will be use for improving or changing the figure. The valid options are:
    ...
    if any(not sim.success for sim in simulation) and not force_unsuccess:
        raise ValueError(
            "The simulation was not successful. Impossible to create figure"
        )
    ...
    if kwds.get("sharex", False):
        xinterval = len(simulation) * [
            (
                min(sim.t.min() for sim in simulation),
                max(sim.t.max() for sim in simulation),
            )
        ]
    else:
    ...
    for i, a in enumerate(
        nditer(ax, ("refs_ok",))
    ):  # iterating by rows through all subplots
        if i >= len(simulation):
            break  # we do not fill all the plots if there are not enough
        a = a.item()  # removing the numpy.ndarray class
        for j in range(len(simulation[i].y)):
            format_to_use = (
                format[i] if not isinstance(format[i], (tuple, list)) else format[i][j]
            )
            a.plot(
                simulation[i].t, simulation[i].y[j], format_to_use, label=names[i][j]
            )
        if i == 0 and legend:
            a.legend()
        a.title.set_text(title[i])
        a.set_xlim(*xinterval[i])
        a.set_ylim(*yinterval[i])
...
def sim_to_IO_format(
    ...
    * ``what``: the type of data we will extract from the simulation. It can be either ``"value"``
    ...
    y = (
        simulation.y.transpose()
    )  # each row is the value of each function at a time from `t`

    if what == "value":  # the data is the next position in time
        output = [
            (y[i], y[i + 1]) for i in range(m - 1) if t[i + 1] - t[i] > 0.9 * tstep
        ]
    elif what == "derivative":  # the data is the derivative of the point
        output = [
            (y[i], (y[i + 1] - y[i]) / (t[i + 1] - t[i]))
            for i in range(m - 1)
            if t[i + 1] - t[i] > 0.9 * tstep
        ]
    else:
        raise ValueError(
            "The requested data (see argument ``what``) is not recognized."
        )
    return output
