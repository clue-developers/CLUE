...
R = PolynomialRing(QQ, 'x', 75)
x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18, x19, x20, x21, x22, x23, x24, x25, x26, x27, x28, x29, x30, x31, x32, x33, x34, x35, x36, x37, x38, x39, x40, x41, x42, x43, x44, x45, x46, x47, x48, x49, x50, x51, x52, x53, x54, x55, x56, x57, x58, x59, x60, x61, x62, x63, x64, x65, x66, x67, x68, x69, x70, x71, x72, x73, x74, x75, = R.gens()
...
#{S0}
result = aggregate(polys, [x1])
print("Total for e3_curry S0 is " + str(len(result)))

#{S1}
result = aggregate(polys, [x2])
print("Total for e3_curry S1 is " + str(len(result)))

#{S33,S34,S35,S36,S44,S45,S46,S47,S55,S56,S2,S3,S4,S5,S6,S7,S9,S10,S11,S12,S13,S14,S18,S19,S20,S21,S22,S23,S24,S25,S31,S32}
result = aggregate(polys, [x34 + x35 + x36 + x37 + x45 + x46 + x47 + x48 + x56 + x57 + x3 + x4 + x5 + x6 + x7 + x8 + x10 + x11 + x12 + x13 + x14 + x15 + x19 + x20 + x21 + x22 + x23 + x24 + x25 + x26 + x32 + x33])
print("Total for e3_curry fr the SUM of Block 3 is " + str(len(result)))
...
result = aggregate(polys, [x38 + x39 + x40 + x41 + x42 + x43 + x49 + x50 + x51 + x52 + x53 + x54 + x55 + x58 + x59 + x60 + x61 + x62 + x63 + x64 + x65 + x66 + x9 + x16 + x17 + x18 + x27 + x28 + x29 + x30 + x31])
print("Total for e3_curry for the SUM of block 4 is " + str(len(result)))
