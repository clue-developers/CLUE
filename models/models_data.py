...
if __name__ == "__main__":
    ...
    data.sort(key = lambda model : (model['type'], model['name']))
    by_types = {}
    for model in data:
    ...
    for type in by_types:
        with open(os.path.join(script_dir,f"{type}/README.mf"), "w") as readme:
            ## First part of the file
            header = (f"## {type} Models\n\n"
            ...
            for model in by_types[type]:
                if(all(el in ("", None) for el in (model['ref_title'], model['ref_doi']))): # no reference
                    ...
                    doi = model_doi(model)
                    readme.write(f"* {model['name']}: [{doi}](https://www.doi.org/{doi})\n")
                elif(model['ref_doi'] in ("", None)): # no doi but title
                    readme.write(f"* {model['name']}: {model['ref_title']}\n")
                else: # both title and doi
                    doi = model_doi(model)
                    readme.write(f"* {model['name']}: [{model['ref_title']}](https://www.doi.org/{doi})\n")
            readme.write("\n")
